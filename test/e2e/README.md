...
The script creates one namespace, applies the manifests, waits for the cluster to settle, runs the checks and deletes the namespace again. Set `KUBEWHY_E2E_YES=1` to skip the confirmation prompt, and `KUBEWHY_E2E_SETTLE` to change how long it waits (default 45 seconds — CrashLoopBackOff and image pull backoff both take time to appear).
